...
Type $(basename "$0") -h for help" >/dev/stderr
  ...
  echo "Did not find menu argument, using \${DEFAULT_MENU}" >/dev/stderr
...
source_dmscripts_configs() {
  ...
  [[ -f "./config.sh" ]] && source "./config.sh" && return 0
  [[ -f "$(dirname $0)/config.sh" ]] && source "$(dirname $0)/config.sh" && return 0
  # [[ -d "${XDG_CONFIG_HOME}" ]] && [[ -f "${XDG_CONFIG_HOME}/dmscripts/config" ]] && source "${XDG_CONFIG_HOME}/dmscripts/config" && return 0
